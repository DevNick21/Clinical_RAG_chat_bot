...
\RequirePackage[l2tabu, orthodox]{nag} % tells you of any bad LaTeX usage
                                       % must be the first thing in class (except comments)

%% There is one option you should define: one-sided or two-sided
%% Use twoside for your viva docs (examiners hate long docs they need to carry around)
%% and one-sided for the final thing you submit to the library.  Note that margins will
%% change accordingly
...
\authorID{24800041}                           % your University Identifier
...
\degree{M.Sc.\ in Artificial Intelligence}                      % the degree you are reading
                                                    % note the \ after the dot, so as not to consider it a full stop
...
{\backmatter
    ...
    {\footnotesize\bibliography{chap1_intro/introduction_biblio,chap2_lit_review/background_and_lit_overview_biblio,chap3_methodology/materials_and_methods_biblio,chap4_results/results_and_discussion_biblio}}
	\printindex
}

\end{document}
git subtree push --prefix=report report master
%%% The End %%%